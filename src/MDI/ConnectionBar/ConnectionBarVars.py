...
def connection_bar_add_sets_and_params(m: ConcreteModel, data) -> ConcreteModel:
    ...
    """
    def _demand_init(m, b, t, p):
        return data.units[b].demand[p][t - 1]
    ...
    m.d = Param(m.CB, m.T, m.P, initialize=_demand_init, within=NonNegativeReals) # demand
...
def connection_bar_add_variables(m: ConcreteModel) -> ConcreteModel:
    ...
    """
    m.D = Var(m.CB, m.T, m.P, domain=NonNegativeReals) # deficit
    if not m.unique_bar:
      m.theta = Var(m.CB, m.T, m.P, domain=Reals)
    return m
