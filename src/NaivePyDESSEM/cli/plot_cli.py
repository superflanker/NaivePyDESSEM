...
def print_welcome_banner():
    ...
    print(f"{Fore.GREEN}{Style.BRIGHT}NaivePlot: {Fore.CYAN}Reporting CLI For {Fore.MAGENTA}NaivePyDESSEM Simulations{Style.RESET_ALL}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="CLI for visualization/export of NaivePyDESSEM results")
    parser.add_argument(
    ...
    categories = [c.upper() for c in (
        args.category or prompt("Enter category(ies) separated by spaces (e.g., G S V F):").split())]
    if mode == "table":
