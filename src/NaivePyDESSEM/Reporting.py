...
def __compute_thermal_generation_cost(model: ConcreteModel) -> float:
    ...
    if has_thermal_model(model):
        generation_cost = sum(
            value(model.thermal_c[g] * model.thermal_p[g, t]**2 +
                  model.thermal_b[g] * model.thermal_p[g, t] +
                  model.thermal_a[g] * model.thermal_u[g, t])
            for g in model.TG for t in model.T
        )
    return generation_cost


def __compute_thermal_start_cost(model: ConcreteModel) -> float:
    """
    Compute total start-up cost for thermal units.

    Parameters
    ----------
    model : ConcreteModel
        Solved Pyomo model with thermal units.

    Returns
    -------
    float
        Total start-up cost [$] for all thermal units over the horizon.
    """
    start_costs = 0.0
    if has_thermal_model(model):
        start_costs = sum(
            value(model.thermal_SC[g] * model.thermal_y[g, t])
            for g in model.TG for t in model.T
        )
    return start_costs
...
def dispatch_summary(model: ConcreteModel) -> None:
    ...
    if all(hasattr(model, attr) for attr in required):
        ...
        if has_thermal_model(model):
            start_cost = __compute_thermal_start_cost(model)
            generation_cost = __compute_thermal_generation_cost(model)
            total_cost = start_cost + generation_cost
            print(
                f"  {Fore.CYAN}Total Thermal Cost{Style.RESET_ALL}: {Fore.RED} $ {format_brl(total_cost)}")
            print(
                f"  {Fore.CYAN}Thermal Start Cost{Style.RESET_ALL}: {Fore.RED} $ {format_brl(start_cost)}")
            print(
