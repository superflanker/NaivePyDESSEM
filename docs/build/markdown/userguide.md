...
NaivePyDESSEM automatically installs **Pyomo** as a dependency.<br />
\\\\
However, certain solvers require Pyomo’s compiled extensions to be available.<br />
\\\\
...
NaivePyDESSEM relies on external solvers for mixed-integer linear programming (MILP) and nonlinear optimization.<br />
\\\\
The following options are supported:

#### **IBM CPLEX**

1. Install IBM ILOG CPLEX Optimization Studio from:<br />
   \\\\
...
1. Install the Gurobi Optimizer from:<br />
   \\\\
...
1. Install SCIP Optimization Suite (open-source alternative):<br />
   \\\\
...
**Note:** Some solvers (e.g., CPLEX, Gurobi) require valid licenses.<br />
\\\\
...
- **DESSEM (Short-Term Hydrothermal Scheduling Model):**<br />
  \\\\
...
- **DECOMP (Medium-Term Hydrothermal Operation Model):**<br />
  \\\\
...
- **MDI (Long-Term Expansion Planning Model):**<br />
  \\\\
...
Each formulation in this chapter includes the complete definition of sets, parameters, decision variables, objective function, and constraints.<br />
\\\\
...
The **NaivePyDESSEM** package follows a layered architecture that mirrors the hierarchical structure of the Brazilian energy models (*MDI*, *DECOMP*, and *DESSEM*).<br />
\\\\
...
Each model variant can be solved directly via its respective command-line entry point.<br />
\\\\
Input data and settings are defined in a YAML file describing system topology, parameters, and solver options.

**DESSEM-like dispatch (short-term)**<br />
\\\\
...
- **`--plot-style`**<br />
  \\\\
  Defines the plot style when `--mode plot` is selected.<br />
  \\\\
  Options:
  - `line` → Line plot.
  - `bar` → Bar plot.
...
- **`--level`**<br />
  \\\\
  Specifies the generation level for categories such as `G` or `GT`.<br />
  \\\\
...
This section provides a structured walkthrough for creating a **NaivePyDESSEM** configuration file (`.yaml`), guiding the user through each key section — from meta information to hydropower, thermal, renewable, and storage data.<br />
\\\\
All keywords, indentation levels, and formats must follow the conventions shown below.

#### 🪶 1. HEADER AND METADATA (`meta`)

Start by defining general model information and global parameters under the `meta:` key.<br />
\\\\
...
Define hydraulic parameters, reservoir characteristics, and inflows for each hydro unit.<br />
\\\\
...
- Cost coefficients follow a quadratic cost function:<br />
  \\\\
  $ C(p) = a + b\,p + c\,p^2 $
...
Define the `bars` and `lines` sections as described below if you are modeling a multi-bar or multi-line system.<br />
\\\\
...
This section presents a detailed guide for constructing a **NaivePyDECOMP** configuration file (`.yaml`), representing a **medium-term hydrothermal operation model** based on the Brazilian DECOMP structure.<br />
\\\\
Each block must respect the same naming conventions, indentation, and parameter formatting as the example below.

#### 🪶 1. HEADER AND METADATA (`meta`)

The `meta` section defines the **simulation scope, solver configuration, and demand series**.<br />
\\\\
...
The **`hydro`** block contains reservoir and flow parameters for all hydroelectric plants.<br />
\\\\
...
This document provides a comprehensive guide for constructing a **NaivePyMDI** configuration file (`.yaml`), used for **generation expansion planning** (long-term).<br />
\\\\
